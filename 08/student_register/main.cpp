    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            // TODO: Add functionality here
                ...
                //if we can find student number in the map then read new phone number
                std::string st_no = parts.at(1);
                if (student_numbers.find(st_no) != student_numbers.end()){

                    //reading new phone number
                    cout<< "Enter a new phone number: ";
                    std::string new_phone_no;
                    getline(std::cin, new_phone_no);
                    cout<<endl;

                    //if number contains alphabet letters
                    if (std::string::npos != new_phone_no.find_first_of("abcdefghijklmnopqrstuvwxyz")) {
                        std::cout << "Erroneous phone number: " <<new_phone_no <<std::endl<<std::endl;
                      }

                    //if number does not contain alphabet letters
                    else{
                        //assigning new phone number to respective student number
                        student_numbers[st_no]->phone_number = new_phone_no;
                        ...
                        //filling the file with map data
                        std::map<string, Student*>::iterator it=user_ids.begin();
                        for (; it != user_ids.end(); it++){
                            myfile << it->second->student_number << ";"<<
                                      it->second->user_id << ";" <<
                                      it->second->name << ";" <<
...
//                student_numbers[command]
//                std::map<string, Student*>::iterator it=student_numbers.begin();
//                for (; it!=student_numbers.end(); ++it){
//                    if (it)
//                }


//                cout<< "Enter a new phone number: ";
//                string phone_number_x;
//                cin>>  phone_number_x;
                 //   if (it.compare(phone_number_x) != 0){
                    //    cout<<"there is such a number";
                     //   }
//                    else
//                    {
